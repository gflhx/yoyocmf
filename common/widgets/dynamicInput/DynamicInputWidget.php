...
class DynamicInputWidget extends InputWidget
{
    ...
    public function init()
    {
        ...
        if (is_string($this->data)) {
            $this->data = \Yii::$app->config->get($this->data);
        }
    ...
    private function parse()
    {
        switch ($this->type) {
            ...
            case 'radio': // 单选
                ...
                break;
            case 'image': // 图片单图
                return UploadWidget::widget(ArrayHelper::merge(['name' => $this->name, 'value' => $this->value, 'onlyUrl' => true,'options'=>['img-width'=>'150px']], $this->widgetOptions));
                break;
            case 'images': // 图片
    ...
    private function parseActive()
    {
        switch ($this->type) {
            ...
            case 'radio': // 单选
                ...
                break;
            case 'image': // 图片
                return UploadWidget::widget(ArrayHelper::merge(['model' => $this->model, 'attribute' => $this->attribute, 'onlyUrl' => true,'options'=>['img-width'=>'150px']], $this->widgetOptions));
                break;
            case 'images': // 图片
...
//                break;
    ...
    public function parseExtra($value)
    {
        ...
        if (\Yii::$app->config->has($value)) {
            return \Yii::$app->config->get($value);
        }
//        foreach (explode("\r\n", $value) as $val) {
        foreach (preg_split("/[\r\n]+/s", $value) as $val) {
            if (strpos($val, '=>') !== false) {
